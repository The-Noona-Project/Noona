...
public class SourceFinder {
    ...
    public static Optional<String> findSource(String chapterUrl) {
        ...
        // Updated regex to match wider patterns
        Pattern pattern = Pattern.compile("(https://[^\\s\"']+/(?:uploads|manga)/[^/]+/\\d{3,4}-001\\.png)");

        try (AutoCloseableExecutor executor = new AutoCloseableExecutor(Executors.newFixedThreadPool(1))) {
            ExecutorService service = executor.executor();
            ...
            service.submit(() -> {
                ...
                try {
                    ...
                    Thread.sleep(2000); // Adjust wait if needed
                    ...
                    for (WebElement img : images) {
                        ...
                        if (src == null || src.isEmpty()) {
                            src = img.getDomProperty("data-src");
                        }
            ...
            }).get(); // Ensure the task is completed before continuing

        } catch (Exception e) {
            log.error("Error during executor shutdown: {}", e.getMessage(), e);
        }

...
class WebtoonsClient(private val ktor: HttpClient) {
    ...
    suspend fun searchSeries(name: String): Pair<Collection<Title>, Collection<Title>> {
        ...
        val webtoons: SearchApiResponse = ktor.get("$MOBILE_BASE_URL/undefined/search/result") {
            parameter("keyword", name); parameter(
            "searchType",
            "WEBTOON"
        ); headers { appendAll(mobileHeaders) }
        }.body()
        val originals: SearchApiResponse = ktor.get("$MOBILE_BASE_URL/undefined/search/result") {
            parameter("keyword", name); parameter(
            "searchType",
            "CHALLENGE"
        ); headers { appendAll(mobileHeaders) }
    ...
    suspend fun getChapters(id: WebtoonsSeriesId): Collection<Episode> {
        ...
        val episodes: EpisodeListApiResponse = ktor.get("$MOBILE_BASE_URL/api/v1/$chaptersPath/$titleNo/episodes") {
            parameter(
                "pageSize",
                "99999"
            ); headers { appendAll(mobileHeaders) }

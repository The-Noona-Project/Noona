...
export class SeriesService {
  ...
  getAllSeriesV2(pageNum?: number, itemsPerPage?: number, filter?: FilterV2<FilterField>, context: QueryContext = QueryContext.None, userId?: number) {
    ...
    return this.httpClient.post<PaginatedResult<Series[]>>(this.baseUrl + 'series/all-v2?context=' + context, data, {
      observe: 'response',
      params
    }).pipe(
      map((response: any) => {
  ...
  getSeriesForLibraryV2(pageNum?: number, itemsPerPage?: number, filter?: FilterV2<FilterField>) {
    ...
    return this.httpClient.post<PaginatedResult<Series[]>>(this.baseUrl + 'series/v2', data, {
      observe: 'response',
      params
    }).pipe(
      map((response: any) => {
  ...
  getCurrentlyReading(userid: number, pageNum: number, itemsPerPage: number) {
    ...
    return this.httpClient.get<PaginatedResult<Series[]>>(this.baseUrl + 'series/currently-reading', {
      observe: 'response',
      params
    }).pipe(
      map((response: any) => {
  ...
  getRecentlyAdded(pageNum?: number, itemsPerPage?: number, filter?: FilterV2<FilterField>) {
    ...
    const data = filter || {};
    return this.httpClient.post<Series[]>(this.baseUrl + 'series/recently-added-v2', data, {
      observe: 'response',
      params
    }).pipe(
      map(response => {
  ...
  refreshMetadata(series: Series, force = true, forceColorscape = true) {
    return this.httpClient.post(this.baseUrl + 'series/refresh-metadata', {
  ...
  scan(libraryId: number, seriesId: number, force = false) {
    return this.httpClient.post(this.baseUrl + 'series/scan', {
      libraryId: libraryId,
      seriesId: seriesId,
      forceUpdate: force
    });
  ...
  getSeriesForTag(collectionTagId: number, pageNum?: number, itemsPerPage?: number) {
    ...
    return this.httpClient.get<PaginatedResult<Series[]>>(this.baseUrl + 'series/series-by-collection?collectionId=' + collectionTagId, {
      observe: 'response',
      params
    }).pipe(
      map((response: any) => {

...
export class AnnotationService {
  ...
  getAllAnnotationsFiltered(filter: FilterV2<AnnotationsFilterField, AnnotationsSortField>, pageNum?: number, itemsPerPage?: number) {
    ...
    return this.httpClient.post<PaginatedResult<Annotation>[]>(this.baseUrl + 'annotation/all-filtered', filter, {
      observe: 'response',
      params
    }).pipe(
      map((res: any) => {
  ...
  exportAnnotations(ids?: number[]) {
    return this.httpClient.post(this.baseUrl + 'annotation/export', ids, {
      observe: 'events',
      responseType: 'blob',
      reportProgress: true
    }).pipe(

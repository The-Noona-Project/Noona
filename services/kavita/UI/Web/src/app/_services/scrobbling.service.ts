...
export class ScrobblingService {
  ...
  updateMalToken(username: string, accessToken: string) {
    return this.httpClient.post<boolean>(this.baseUrl + 'scrobbling/update-mal-token', {
      username,
      accessToken
    }, TextResonse)
      .pipe(map(r => r + '' === 'true'));
  ...
  getScrobbleEvents(filter: ScrobbleEventFilter, pageNum: number | undefined = undefined, itemsPerPage: number | undefined = undefined) {
    ...
    return this.httpClient.post<PaginatedResult<ScrobbleEvent[]>>(this.baseUrl + 'scrobbling/scrobble-events', filter, {
      observe: 'response',
      params
    }).pipe(
      map((response: any) => {

...
export class MetadataService {
  ...
  getGenreWithCounts(pageNum?: number, itemsPerPage?: number) {
    ...
    return this.httpClient.post<PaginatedResult<BrowseGenre[]>>(this.baseUrl + 'metadata/genres-with-counts', {}, {
      observe: 'response',
      params
    }).pipe(
      map((response: any) => {
  ...
  getTagWithCounts(pageNum?: number, itemsPerPage?: number) {
    ...
    return this.httpClient.post<PaginatedResult<BrowseTag[]>>(this.baseUrl + 'metadata/tags-with-counts', {}, {
      observe: 'response',
      params
    }).pipe(
      map((response: any) => {
  ...
  private getAnnotationOptionsForFilterField(field: AnnotationsFilterField): Observable<{
    value: number,
    label: string,
    color?: RgbaColor
  }[]> {
      ...
      case AnnotationsFilterField.HighlightSlots:
        return of(this.highlightSlots().map((slot, idx) => {
          return {
            ...
            label: translate('highlight-bar.slot-label', {slot: slot.slotNumber + 1}),
            color: slot.color
          }; // Slots start at 0
        }));
  ...
  private getPersonOptionsForFilterField(field: PersonFilterField) {
    switch (field) {
      case PersonFilterField.Role:
        return of(allPeopleRoles.map(r => {
          return {value: r, label: this.personRolePipe.transform(r)}
        }));
    }
    return of([])
  }

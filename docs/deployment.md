...
* Cleanup calls only target images in the `docker.darkmatterservers.com/the-noona-project/noona-*` namespace (or entries on the internal allowlist) and lifecycle history now records the canonical image tag once per run so repeated digests do not clutter `deployment/lifecycleHistory.json`.
